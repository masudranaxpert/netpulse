...
const EMPTY = {
  ...
  snmp_community: "public", timeout: "10",
};
...
export function OltDeviceForm({ open, onClose, initial, onSubmit, submitting, error }: Props) {
  ...
  useEffect(() => {
    ...
    setF(initial ? {
      ...
      snmp_community: initial.snmp_community ?? "public", timeout: String(initial.timeout ?? 10),
    } : { ...EMPTY });
  }, [open, initial]);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      snmp_community: f.snmp_community, timeout: Number(f.timeout) || 10,
    });
  };
